        ...
        <div className="pt-32 pb-32 flex-1">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto text-center">
              <h1 className="text-4xl md:text-6xl font-light text-architectural mb-8">{t("blog.postNotFound")}</h1>
              <Link to="/blog" className="text-minimal text-foreground hover:text-muted-foreground transition-colors duration-300">{t("blog.backToBlog")}</Link>
            </div>
          </div>
        </div>
        <Footer />
            ...
            <div className="prose prose-lg max-w-none">
              <div className="text-muted-foreground leading-relaxed space-y-6" dangerouslySetInnerHTML={{
                __html: post.content.split('\n').map(line => {
                  if (line.startsWith('# ')) return `<h1 class="text-3xl md:text-4xl font-light text-architectural mb-8 mt-12">${line.substring(2)}</h1>`;
                  else if (line.startsWith('## ')) return `<h2 class="text-2xl md:text-3xl font-light text-architectural mb-6 mt-10">${line.substring(3)}</h2>`;
                  else if (line.startsWith('### ')) return `<h3 class="text-xl md:text-2xl font-medium text-foreground mb-4 mt-8">${line.substring(4)}</h3>`;
                  else if (line.startsWith('- **') && line.endsWith('**')) return `<li class="ml-6 mb-2"><strong class="text-foreground">${line.substring(4, line.length - 2)}</strong></li>`;
                  else if (line.startsWith('- ')) return `<li class="ml-6 mb-2">${line.substring(2)}</li>`;
                  else if (line.trim() === '') return '<br>';
                  else if (line.startsWith('**') && line.endsWith('**')) return `<p class="mb-4"><strong class="text-foreground">${line.substring(2, line.length - 2)}</strong></p>`;
                  else return `<p class="mb-4">${line}</p>`;
                }).join('')
              }} />
            </div>
            <div className="mt-16 pt-8 border-t border-border">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">{t("blog.architectWriter")}</p>
                </div>
              </div>
            </div>
            <div className="mt-20">
              <h3 className="text-2xl font-light text-architectural mb-8">{t("blog.relatedArticles")}</h3>
