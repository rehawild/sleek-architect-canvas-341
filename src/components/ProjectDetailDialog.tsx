      ...
      <DialogContent className="max-w-5xl w-[95vw] max-h-[90vh] overflow-y-auto p-0 gap-0">
        ...
        <div className="relative aspect-[2/1] w-full bg-muted flex items-center justify-center">
          {currentItem?.type === "video" ? (
            <video src={currentItem.src} className={`max-w-full max-h-full object-contain ${project.grayscale && !currentItem.colorOn ? "grayscale" : ""}`} controls autoPlay muted />
          ) : currentItem ? (
            <img src={currentItem.src} alt={`${project.title} - Image ${currentImageIndex + 1}`} className={`max-w-full max-h-full object-contain ${project.grayscale && !currentItem.colorOn ? "grayscale" : ""}`} />
          ) : null}
        ...
        {project.gallery.length > 1 && (
          <div className="flex justify-center bg-muted/30 border-y border-border/50 py-[4px]">
            {showDots ? (
              <div className="flex gap-2 px-4 py-2">
                {project.gallery.map((_, index) => (
                  <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-200 ${index === currentImageIndex ? "bg-foreground scale-125" : "bg-foreground/30 hover:bg-foreground/50"}`} aria-label={`Go to item ${index + 1}`} />
                ))}
              </div>
            ) : (
        ...
        <div className="p-8 md:p-12 space-y-8">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div>
              <h2 className="text-3xl md:text-4xl font-light text-architectural mb-2">{project.title}</h2>
              <p className="text-xl text-muted-foreground">{project.city}</p>
            </div>
            <div className="text-right">
              <span className="text-minimal text-muted-foreground">{t("projectDialog.year")}</span>
          ...
          <div className="grid md:grid-cols-2 gap-8 pt-6 border-t border-border">
            <div>
              <h3 className="text-minimal text-muted-foreground mb-4">{t("projectDialog.clients")}</h3>
              <ul className="space-y-2">
                {project.clients.map((client, index) => <li key={index} className="text-foreground">{client}</li>)}
              </ul>
            </div>
            <div>
              <h3 className="text-minimal text-muted-foreground mb-4">{t("projectDialog.services")}</h3>
              <ul className="space-y-2">
